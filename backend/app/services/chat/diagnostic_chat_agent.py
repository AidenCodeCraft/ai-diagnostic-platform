...
class DiagnosticChatAgent:
    ...
    def enrich_messages(
        ...
        Args:
            ...
            existing_messages: Already-built message history (system + user + assistant)
            log_analysis: Optional pre-computed log analysis result

        Returns:
            Enriched messages list ready for the LLM provider
        """
        ...
        # 4. Replace system message with enriched version, keep history
        enriched: List[Dict[str, str]] = [
            {"role": "system", "content": system_prompt}
        ]
        # Skip the original system message, add the rest
